...
function parseCliArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '-n' || args[i] === '--num') {
      ...
      output = args[i + 1] || '';
      i++;
    } else if (args[i] === '-t' || args[i] === '--tsn') {
      tsn = parseInt(args[i + 1]) || 0;
      i++;
 ...
 */
async function searchWechatArticles(query, maxResults = 10, tsn = 0) {
  // 限制最大结果数
  ...
  while (articles.length < maxResults && page <= pagesNeeded) {
    try {
      ...
      const tsnParam = tsn > 0 ? `&tsn=${tsn}` : '';
      const url = `https://weixin.sogou.com/weixin?query=${encodedQuery}&s_from=input&_sug_=n&type=2${tsnParam}&page=${page}&ie=utf8`;
...
async function main() {
  ...
  const { query, num, output, tsn } = parseCliArgs(args);
...
选项:
  ...
  -t, --tsn <范围>       时间范围筛选（0=不限, 1=1天, 2=3天, 3=1周, 4=1月, 5=3月）

示例:
  node search_wechat.js "人工智能" -n 20
  node search_wechat.js "ChatGPT" -n 10 -o result.json
  node search_wechat.js "跨境电商" -n 10 -t 5
`);
    process.exit(0);
  }
  ...
  try {
    ...
    const articles = await searchWechatArticles(query, num, tsn);
